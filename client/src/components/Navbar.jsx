    ...
    <nav className="flex justify-between items-center px-6 sm:px-10 md:px-14 lg:px-18 py-2 shadow-md">
      ...
      <div className="hidden md:flex items-center gap-6 ">
        <ul className="flex w-full gap-6">
          {menuItems.map((item) => (
            <li className="px-3 sm:text-sm md:text-md lg:text-xl text-secondary hover:text-primary transition duration-500  font-medium"><Link to={item.link}>{item.title}</Link></li>
          ))}
        </ul>
          {/* ----------------- Call to Action ----------------  */}
